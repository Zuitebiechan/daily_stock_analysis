    ...
    def _send_telegram_message(
        ...
        for attempt in range(1, max_retries + 1):
            ...
            if response.status_code == 200:
                ...
                # HTTP 400 通常是因为 Markdown 解析失败
                result = response.json()
                error_desc = result.get("description", "未知错误")
                logger.error(f"Telegram 返回 400 错误: {error_desc}")

                # 如果是 Markdown 解析失败，退回到纯文本发送
                if (
                    "parse" in error_desc.lower()
                    or "markdown" in error_desc.lower()
                    or "entity" in error_desc.lower()
                ):
                    logger.info("尝试使用纯文本格式重新发送...")
                    plain_payload = dict(payload)
                    plain_payload.pop("parse_mode", None)
                    plain_payload["text"] = text  # 使用原始文本

                    try:
                        response_plain = requests.post(
                            api_url, json=plain_payload, timeout=10
                        )
                        if (
                            response_plain.status_code == 200
                            and response_plain.json().get("ok")
                        ):
